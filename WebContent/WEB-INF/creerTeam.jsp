			...
			<form method="post" action="<c:url value="/creationTeam"/>">
				...
				<c:if test="${ !empty sessionScope.pilotes }">
					<div class="custom-control custom-radio custom-control-inline">
						<label class="radio-inline" for="choixNouveauClient">Nouveau
							pilote ? <span class="requis">*</span> <input type="radio"
							id="choixNouveauPilote" name="choixNouveauPilote"
							value="nouveauPilote" checked /> Oui <input class="radio-inline"
							type="radio" id="choixNouveauPilote" name="choixNouveauPilote"
							value="ancienPilote" /> Non <br /> <br />
						</label>
				</c:if>
		</div>
			...
			<div id="ancienPilote">
				<div class="lolo col-lg-8 col-lg-offset-2">
					<div class="form-group">
						<select class="form-control form-control-lg" name="listePilotes"
							id="listePilotes">
							...
							<c:forEach items="${ sessionScope.pilotes }" var="mapPilotes">
								...
								<option value="${ mapPilotes.value.nom }">${ mapPilotes.value.prenom }
									${ mapPilotes.value.nom }</option>
							</c:forEach>
						</select>
					</div>
				</div>
